  ...
  return (
    ...
    {/* <div class="tour col-lg-4 col-md-6 animate__animated animate__fadeInRight"> */}
      ...
      <div class="title">{country.city}</div>
                ...
                <div>
        <h4>{country.price} руб.</h4>
                </div>
    
                </div>
    
              <img src={country.photoUrl} alt='hotelImg' width='300px' height='240px'/>
      
         <div>
      <span>
      <h4>{country.temperature} °С</h4>
      </span>&ensp;&ensp;&ensp;
    <span>
      <div>
            <a href={country.url}>Go to tour</a>
    
      </div>
      <div>
        { addShow ?
            <button onClick={() => addTourHandler(user._id, country._id)} class="add-tour-button">Добавить в избранное</button> :
            
            <button onClick={() => deleteTourHandler(user._id, country._id)} class="delete-tour-button" >Remove from my profile</button>
    } 
          </div>
